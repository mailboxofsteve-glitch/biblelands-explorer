    ...
    <header className="border-b border-border/40 px-4 md:px-6 py-3 flex items-center justify-between bg-card/80 backdrop-blur-sm shrink-0 z-30">
      <div className="flex items-center gap-4">
        ...
        >
          ...
          <span className="text-lg font-serif font-bold tracking-wide text-foreground">BibleLands</span>
        </button>
        ...
        <nav className="hidden sm:flex items-center gap-1">
          {navLinks.map((link) => (
            <Button
              ...
              className={location.pathname === link.path ? "text-foreground font-semibold" : "text-muted-foreground"}
            >
              {link.label}
            </Button>
          ))}
      ...
      <div className="flex items-center gap-1.5">
        {isAdmin && (
          <Button variant="ghost" size="sm" onClick={() => navigate("/admin")} className="text-muted-foreground gap-1">
            <Shield size={14} />
